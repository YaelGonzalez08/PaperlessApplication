...
class ClientesCtrlAbordajeFragment(var fligthReference: String) : Fragment() {
    ...
    private fun updatePasajeros() {
        ...
        model.getBoardedPasajeros(fligthReference).observe(viewLifecycleOwner, Observer {
            d.Ocultar()
            if(it!=null) {
                var resp = it
                if (resp !=null) {
